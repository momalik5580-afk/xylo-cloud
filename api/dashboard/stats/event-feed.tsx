...
export function RealTimeEventFeed() {
  ...
  return (
    <Card className="border border-slate-800/60 bg-slate-900/80 overflow-hidden">
      <div
        ref={scrollRef}
        className="flex items-center gap-6 py-3 px-4 overflow-x-hidden"
      ...
      >
        {[...events, ...events].map((event, index) => {
          ...
          return (
            <div key={`${event.id}-${index}`} className="flex items-center gap-3 flex-shrink-0 group cursor-pointer">
              <span className="text-xs font-mono text-slate-500">{event.timestamp}</span>
              <div className={cn("p-1.5 rounded-lg bg-slate-950/50", typeColors[event.type])}>
                <Icon className="h-3.5 w-3.5" />
              </div>
              <span className="text-sm text-slate-300 group-hover:text-[#8E939D] transition-colors whitespace-nowrap">
                {event.message}
              </span>
              <div className="h-4 w-[1px] bg-slate-800 ml-3" />
            </div>
          )
        })}
      </div>
